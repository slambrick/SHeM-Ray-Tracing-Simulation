...
classdef TriagSurface < handle
    ...
    properties (SetAccess = private)
        ...
        parameters  % Parameters for the scattering from each triangle
        nTriag      % The number of triangles in the surface
        nVertices   % The number of vertices in the surface
    end % End properties
    
    methods
        function obj = TriagSurface(V, N, F, C, P)
        ...
        %  obj - A TriagSurface object that contains the surface.
            ...
            elseif (nargin == 5)
                ...
                if (sum(~(C <= 4)) ~= 0 || sum(~(C >= 0)))
                    error('The diffusivity level can only be betwen 0 and 4')
                end
                obj.vertices = V;
                obj.normals = N;
                obj.faces = F;
        ...
        % Copys the object and returns the copy.
            newobj = TriagSurface(obj.vertices, obj.normals, obj.faces, ...
                                  obj.composition, obj.parameters);
        end % End copy method
