 ...
 * 
 *  INPUTS:
 *   numScattersRay - 1D column array to bin, must have more than two elements
 *   maxScatters    - The number to bin up to, must be an integer greater than 1
 * 
 *  OUTPUTS
 *   histRay - 1D array of the binned variables
 * 
 *  EXAMPLE:
 *   % The call
...
void binIt(double numScattersRay[], int histRay[], int maxScatters, int nRays) {
    int i, j;
    
    for (i = 0; i < nRays; i++) {
        for (j = 1; j <= maxScatters; j++) {
            if (j == (int)numScattersRay[i]) {
                histRay[j - 1] += 1;
            }
        }
    }
...
void mexFunction(int nlhs, mxArray *plhs[], 
    ...
    if (!mxIsDouble(prhs[0]) || mxIsComplex(prhs[0])) {
        mexErrMsgIdAndTxt("MyToolbox:binMyWayMex:notDouble", 
                          "Input vector must be of type double.");
    }
